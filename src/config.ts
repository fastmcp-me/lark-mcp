...
function parseArgs(): Partial<LarkConfig> {
  const args = process.argv.slice(2);
  const config: Partial<LarkConfig> = {};
  
  // Use positional arguments - first is appId, second is appSecret
  if (args.length >= 1) config.appId = args[0];
  if (args.length >= 2) config.appSecret = args[1];
...
export const config: LarkConfig = {
